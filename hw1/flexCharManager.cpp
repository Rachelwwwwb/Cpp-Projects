...
char* flexCharManager::alloc_chars(int n){
    ...
    while (istrue){
            ...
            if (i == active_requests-1){  
                if (used_memory[i]->physical_location + used_memory[i]->size + n 
                < buffer + BUF_SIZE){
                    returnval = used_memory[i]->physical_location + 
                    used_memory[i]->size;
                    break;}
                else{
                    return NULL;}
            ...
            if (used_memory[i]->physical_location + used_memory[i]->size + n > 
            used_memory[i+1]->physical_location){
                i++;}
            else if (used_memory[i]->physical_location + used_memory[i]->size + n 
            <= used_memory[i+1]->physical_location){
    ...
    if (active_requests >= used_mem_size){
        ...
        int j = 0;
        for (int h = 0; h < active_requests; h++){
            temp[j] = used_memory[h];
            j++;
        }
        delete[] used_memory; 
        used_memory = temp;
